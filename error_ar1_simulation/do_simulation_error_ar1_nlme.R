simulation_error_ar1_nlme <- function(data, pred_data, filename, feature_strings, re_data_nlme, re_ops, re_params, 
  ...
  for (i in 1:sims) {
    ...
    if(parallel) {
      re_par <- gmjmcmc.parallel.re(runs = runs, cores = cores,
                                    ...
                                    re_data = re_data_nlme,
                                    rand_effects = re_ops,
                                    marg_lik_method = marg_lik_method,
                                    verbose = verbose)

      preds <- predict.gmjmcmc_merged.nlme(re_par, pred_data, re_data_nlme)
      ...
      preds <- predict.gmjmcmc.nlme(re_par, pred_data, re_data_nlme)
  ...
  params_vec <- c(tot_run_time, ar_par, ar_sd, ar_tr, n.ini, n.fin, pops, noise, thres, simus, thres, 
                  run_parallel, rs, cs, add_l_resp, rand_sed, space)
