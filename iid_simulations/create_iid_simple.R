create_iid_simple <- function(n, ma, sd, noise_sd, ar_trend, ar_par, ar_sd, train_prop=0, seed=100){
  set.seed(seed)
  ...
  ma_1 <- ma + 1
  y <- rep(NA, n)
  y[ma_1] <- 0 + feature1[ma_1] + feature2[ma_1] + feature3[ma_1] + feature4[ma_1]
  for (i in (ma_1+1):n) {
    y[i] <- feature1[i] +  feature2[i] + feature3[i] + feature4[i]
  }
