simulation_iid <- function(data, pred_data, filename, feature_strings,
                       transforms, ts_transforms, window_list, params, probs, P, N.init, N.final, add_lagged_response,
                       noise_sd, sims, runs, cores, threshold = 0.5, seed = 100, parallel = FALSE,
                       verbose = FALSE) {
  ...
  for (i in 1:sims) {
    ...
    set.seed(seed+i)
    if(parallel) {
      ts_par <- gmjmcmc.parallel.ts(runs = runs, cores = cores,
                                    data_ts = data,
      ...
      preds <- predict.gmjmcmc_merged.ts(ts_par, pred_data)
      feat_df <- summary.gmjmcmc_merged.ts(ts_par, labels = var_names)
    }
    else{
      ts_par <- gmjmcmc.ts(
        data_ts = data,
      ...
      preds <- predict.gmjmcmc.ts(ts_par, pred_data)
      feat_df <- summary.gmjmcmc.ts(ts_par, labels = var_names)
    }
  ...
  space <- paste("\n#######################", sep="")
  params_vec <- c(tot_run_time, n.ini, n.fin, pops, noise, thres, simus, thres, run_parallel, rs, cs, add_l_resp, rand_sed, space)
